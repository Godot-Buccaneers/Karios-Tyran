class_name DragInfo extends Node2D
	...
	if event is InputEventScreenTouch:
		...
		if event.is_pressed():
			...
			if event.button_index == MOUSE_BUTTON_WHEEL_UP:
				zoom += Vector2(1,1) * event.factor #* FakeDelta.delta
			# zoom out
			if event.button_index == MOUSE_BUTTON_WHEEL_DOWN:
				zoom -= Vector2(1,1) * event.factor
	
			zoom = clampf(zoom.length(),0,10) * zoom.normalized().abs()
...
func handle_drag(event: InputEventScreenDrag):
	...
	if touch_points.size() == 1:
		...
		zoom = clampf(zoom.length(),0,10) * zoom.normalized()
		#limit_zoom(zoom)
